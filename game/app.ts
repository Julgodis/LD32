...
window.onload = () => {
    ...
    data.shaders["light"] = Loader.ReadShaderResource(
        "shaders/light.vert",
        "shaders/light.frag",
        {
            "lightCount": {
...
function start() {
    ...
    // NOTE: Title
    {
        ...
        var object: any = new Tile(new vec2([worldBound.x / 2 - 70 * 4 * ppm, worldBound.y / 2 - 1]), new vec2([140 * 4, 30 * 4]).mul(ppm));
    ...
    // NOTE: Play button
    {
        ...
        var object: any = new Tile(new vec2([worldBound.x / 2 - 40 * 2 * ppm, worldBound.y - worldBound.y / 3]), new vec2([80 * 2, 38 * 2]).mul(ppm));
        object.sprite = sprite;
        object.isStatic = true;
...
function startGame() {
    ...
    for (var x = 0; x < 80; x++) {
        ...
        var change = 0;
        var rnd = Math.random();
        if (rnd > 0.80) change = 1;
        if (rnd < 0.30) change = -1;
        if (rnd < 0.10 && yLevel >= 2) change = -2;
        if (rnd < 0.05 && yLevel >= 3) change = -3;
        if (rnd < 0.02 && yLevel >= 4) change = -4;
        if (rnd < 0.01 && yLevel >= 5) change = -5;
...
function addPlayer() { // player
    ...
    var object: any = new Player(new vec2([-3, 1]), new vec2([4 * 7, 20 * 7]).mul(ppm));
    object.offset = new vec2([8 * 7, 0]).mul(ppm);
    object.sprite = sprite;
    object.isStatic = false;
    object.velocity.y = -1;
...
function addEnemy(x: number, y: number)
{
    ...
    var object: any = new Enemy(new vec2([x, y]), new vec2([4 * 7, 20 * 7]).mul(ppm));
    object.offset = new vec2([8 * 7, 0]).mul(ppm);
    object.sprite = sprite;
    object.isStatic = false;
    object.velocity.y = -1;
...
{ // the rock
    ...
    var object: any = new Rock(new vec2([player.position.x + 2, 2]), new vec2([sprite.width, sprite.height]).mul(ppm), sprite.width * 0.5 * ppm);
    var rock = <Rock>object;
    object.sprite = sprite;
    object.isStatic = false;
    object.velocity.y = -1;
    object.velocity.x = 0;
...
function addSlope(x: number, y: number, dir: boolean) {
    ...
    var object: any = new TileSlope(new vec2([x, y]), new vec2([128, 128]).mul(ppm), angle);
    object.sprite = sprite;
    object.isStatic = true;
...
function addBlock(x: number, y: number) {
    ...
    var object: any = new Tile(new vec2([x, y]), new vec2([128, 128]).mul(ppm));
...
function addTorch(x: number, y: number, t: number = 10) {
    ...
    var light = new Light(
        new vec3([x * mpp, y * mpp, 45]),
        300,
        t,
        new vec3([1.0, 0.7, 0.5]));
    light.torch = true;
    lights.push(light);
...
function addBlockStyle(x: number, y: number) {
    ...
    var style: any = new Style(sprite);
    style.x = x * mpp;
    style.animate = false;
    style.width = 16;
    style.height = 16;
...
function addGround(y: number) {
    ...
    var object: any = new Tile(new vec2([-500 * 128 * ppm, y]), new vec2([sprite.width, sprite.height]).mul(ppm));
    object.sprite = sprite;
    object.isStatic = true;
...
function update() {
    ...
    if (player != null) {
        lightsVisible = lightsVisible.sort((a: Light, b: Light) => {
    ...
    var toUpdate = objects.filter((value) => {
        if (player == null) {
        ...
        var a =  (value.min.x >= player.min.x - worldBound.x
            && value.max.x <= player.max.x + worldBound.x) || (value.min.x <= player.min.x - worldBound.x
                && value.max.x >= player.max.x + worldBound.x) || (value.min.x >= player.min.x - worldBound.x
                && value.min.x <= player.max.x + worldBound.x) || (value.max.x >= player.min.x - worldBound.x
                && value.max.x <= player.max.x + worldBound.x);
    ...
    for (var id in toUpdate) {
        ...
        {   
            if (obj instanceof Player) {
                ...
                if (player.hp < 0) {
                    ...
                    for (var i = 0; i < 10; i++) {
                        ...
                        var object: any = new Dead(player.position.copy(), new vec2([6 * 7, 6 * 7]).mul(ppm), 3 * 7 * ppm);
                ...
                if (enemy.hp < 0) {
                    ...
                    for (var i = 0; i < 10; i++) {
                        ...
                        var object: any = new Dead(enemy.position.copy(), new vec2([6 * 7, 6 * 7]).mul(ppm), 3 * 7 * ppm);
                        object.offset = new vec2([0, 0]).mul(ppm);
                        object.sprite = sprite;
                        object.isStatic = false;
                        object.velocity.y = -(Math.random() * 5 + 2);
            ...
            for (var id2 in toUpdate) {
                ...
                if (obj2 instanceof Dead) continue;

                if (obj2 instanceof Rock) {
                    if (obj instanceof Dead) continue;

                    if (!(obj.max.x > obj2.min.x && obj.min.x < obj2.max.x)
                        && (obj.max.y > obj2.min.y && obj.min.y < obj2.max.y))
                        continue;
                    ...
                    obj2 instanceof TileSlope &&
                    obj2.isStatic)
                {
                    var slope = <TileSlope>obj2;
                    ...
                    //} else {
                        ...
                        var f = (x: number) => { return x }; // function

                        var collide = (obj.max.x > obj2.min.x && obj.min.x < obj2.max.x)
                                        && (obj.max.y > obj2.min.y && obj.min.y < obj2.max.y);
...

                             {
                                ...
                                if (obj instanceof Rock || obj instanceof Dead) {
                                    ...
                                    if (obj instanceof Rock) {
                                        radius = (<Rock>obj).radius;
                                    } else {
                                        radius = (<Dead>obj).radius;
                                    }
                                    ...
                                    if (obj.position.y >= obj2.max.y - y - obj.size.y &&
                                        (obj.position.y - (obj2.max.y - y - obj.size.y)) < (obj.size.y / 2))
                                    {
                    ...
                    if (obj.max.x + edge > obj2.min.x && obj.min.x - edge < obj2.max.x) {
                        if (obj.max.y + edge >= obj2.min.y && obj.min.y - edge <= obj2.max.y) {
                            ...
                            if (dposition.x < dposition.y) direction.y = 0;
                            else direction.x = 0;
                            ...
                            if (edge == 0)
                                ...
                                collide = (obj.max.x > obj2.min.x && obj.min.x < obj2.max.x)
                                && (obj.max.y >= obj2.min.y && obj.min.y <= obj2.max.y);
                            }
           ...
           /* if (obj.max.x > worldBound.x) {
                obj.position.x = worldBound.x - obj.size.x;
            ...
            if (obj.max.y > worldBound.y) {
                obj.position.y = worldBound.y - obj.size.y;
    ...
    objects = objects.filter((value: GameObject, index: number, array: GameObject[]) => {
        return !value.remove;
    });

    var toStyle = styles.filter((value) => {
        if (player == null) {
            value.sprite.enabled = false;
            return false;
        }
